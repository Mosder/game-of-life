...
int* count_neighbors_row(int width, char *row, char *row_above, char *row_below) {
	...
	// array which contains neighbor data for the whole byte, one entry one neighbor data, 8 neighbors total
	...
	for (int byte = 0; byte < width_bytes; byte++) {
		...
		// determine if last byte, since no next byte and clipping needed for this byte
...
char* decide_fate_row(char *row, int width, int *counts) {
	...
	for (int byte = 0; byte < width_bytes; byte++) {
		...
		// if 3 neighbors - always alive next frame and also stays alive if at least 2 neighbors
...
char* convolve_conway(char *bitmap, int width, int height) {
	...
	// prev row of zeros to not handle first row seperately
	char *prev_row = zeros;
	for (int r = 0; r < height; r++) {
		int *counts;
		// Determine if it's the last row, because not new row after last (duh)
		if (r != height-1)
